...
/* Sets up a VEC3D world, but one including visitors from 
   other machines.
...
public class Content3D extends JPanel implements  ActionListener
// Holds the 3D canvas where the loaded image is displayed
{
  ...
  private static final int WIDTH = 650;     // size of panel
  private static final int HEIGHT = 420; 
  private JPanel buttonPanel,TextAreaPane;
  private JTextField TextAreaField;
  private JButton jbTalk;
  private VoiceChat vcTalk;
  ...
  // construct the 3D canvas
  {
    ...
    TextAreaField = new JTextField("hi !", 8);
    TextAreaPane.add(new JLabel("對話訊息："));
    ...
    // function table & user list (right)
    javax.swing.Box funct = javax.swing.Box.createVerticalBox();
	
	JLabel lt = new JLabel("功能選項");
	funct.add(lt);

	jbTalk = new JButton("對話功能");
	jbTalk.addActionListener(this);
	funct.add(jbTalk);
	funct.add(javax.swing.Box.createVerticalStrut(5));
	
    for (int i = 0;i < 4 ;i++){
    	funct.add(new JButton(""+i));
    	funct.add(javax.swing.Box.createVerticalStrut(5));
	}

	JLabel lb = new JLabel("線上使用者");
	funct.add(lb);
	JTextArea UserList = new JTextArea(userName,10,10);
    ...
    // virtual world (left)
    canvas3D.setSize(new Dimension(WIDTH, HEIGHT));
  ...
  public void actionPerformed(ActionEvent e) {
    if(e.getSource()==jbTalk)
      vcTalk = new VoiceChat(""+2000, "192.168.0.1", ""+2000);
